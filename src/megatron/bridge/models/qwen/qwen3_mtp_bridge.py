...
class Qwen3MTPBridge(Qwen3Bridge):
    def mapping_registry(self) -> MegatronMappingRegistry:
        ...
        mtp_mappings = [
            ...
            # Transformer Layer
            AutoMapping("mtp.layers.*.transformer_layer.self_attention.linear_qkv.layer_norm_weight",
                        "mtp.*.transformer_layer.input_layernorm.weight"),
            AutoMapping("mtp.layers.*.transformer_layer.mlp.linear_fc1.layer_norm_weight",
                        "mtp.*.transformer_layer.post_attention_layernorm.weight"),

            # Qwen3 Specific Norms
            AutoMapping("mtp.layers.*.transformer_layer.self_attention.q_layernorm.weight",
                        "mtp.*.transformer_layer.self_attn.q_norm.weight"),
            AutoMapping("mtp.layers.*.transformer_layer.self_attention.k_layernorm.weight",
                        "mtp.*.transformer_layer.self_attn.k_norm.weight"),

            # Projections
            AutoMapping("mtp.layers.*.transformer_layer.self_attention.linear_proj.weight",
                        "mtp.*.transformer_layer.self_attn.o_proj.weight"),
            AutoMapping("mtp.layers.*.transformer_layer.mlp.linear_fc2.weight",
        ...
        special_mappings = [
            ...
            GatedMLPMapping(
                megatron_param="mtp.layers.*.transformer_layer.mlp.linear_fc1.weight",
